...
class DownloadSiteTag(_PluginBase):
    ...
    # 插件描述
    plugin_desc = "自动给下载任务分类与打站点标签、媒体名称标签，支持电影、电视、动漫和音乐"
    # 插件图标
    plugin_icon = "Youtube-dl_B.png"
    # 插件版本
    plugin_version = "3.3.0"
    # 插件作者
    plugin_author = "叮叮当 / helios"
    # 作者主页
    ...
    def init_plugin(self, config: dict = None):
        ...
        if config:
            ...
            self._category_music = config.get("category_music") or "音乐"
            ...
            for torrent in torrents:
                try:
                    ...
                    # 如果站点名称为空, 尝试通过trackers识别
                        ...
                        for tracker in trackers:
                            ...
                            if site_info:
                                ...
                                break
                        # 如果通过 tracker 仍无法识别站点，且媒体身份、类型、标题均为空，则跳过当前种子。
                        if (not history.torrent_site
                                and not history.media_id
                                and not history.type
                                and not history.title):
                            continue
                    # 按设置生成需要写入的标签与分类
                    _tags = []
                    _cat = None
                    ...
                    if service.type == "qbittorrent" and self._enabled_category and not torrent_cat and history.type:
                        # 如果是电视剧 需要区分是否动漫
                        genre_ids = None
                        # 仅 TMDB 主身份可调用 TMDB 分类接口。
                        history_type = MediaType(history.type) if history.type else None
                        if (history.media_source == MediaSource.TMDB
                                and history.media_id
                                and history_type == MediaType.TV):
                            tmdb_info = self.chain.tmdb_info(
                                mtype=history_type,
                                tmdbid=history.media_id,
                            )
                            if tmdb_info:
    ...
    @staticmethod
    def _is_media_type(mtype, expected: MediaType) -> bool:
        """兼容枚举、中文值和少量英文别名。"""
        if mtype is None:
            return False
        aliases = {expected, expected.value}
        if expected == MediaType.MUSIC:
            aliases.add("music")
        return mtype in aliases

    def _genre_ids_get_cat(self, mtype, genre_ids=None):
        """
        按媒体类型映射下载器分类；电视剧再按 genre_ids 区分动漫。
        """
        if self._is_media_type(mtype, MediaType.MOVIE):
            return self._category_movie
        if self._is_media_type(mtype, MediaType.MUSIC):
            return self._category_music
        if self._is_media_type(mtype, MediaType.TV):
            anime_genre_ids = settings.ANIME_GENREIDS
            if genre_ids and set(genre_ids).intersection(set(anime_genre_ids)):
                return self._category_anime
            return self._category_tv
        return None
    ...
    def download_added(self, event: Event):
        ...
        try:
            ...
            media_title = None
            if _media:
                media_title = _media.title or getattr(_media, "album", None)
            if self._enabled_media_tag and media_title:
                media_tag = self._generate_media_tag(media_title)
                _tags.append(media_tag)
            # 分类, 如果勾选开关的话 <tr暂不支持>
            if self._enabled_category and _media and _media.type:
                genre_ids = getattr(_media, "genre_ids", None)
                _cat = self._genre_ids_get_cat(_media.type, genre_ids)
                                ...
                                'content': [
                                    {
                                        'component': 'VTextField',
                                        'props': {
                                            'model': 'category_music',
                                            'label': '音乐分类名称(默认: 音乐)',
                                            'placeholder': '音乐',
                                            'hint': 'V3 音乐下载任务使用该分类，请填写下载器里已创建的音乐分类名称'
                                        }
                                    }
                                ]
                            }
