...
class MaoyanRank(_PluginBase):
    ...
    def get_page(self) -> List[dict]:
        ...
        for index, history in enumerate(historys, start=1):
            ...
            items.append({
                ...
                "tmdb_href": tmdb_href(
                    media_type,
                    history.get("media_id")
                    if normalize_media_source(history.get("media_source")) == MediaSource.TMDB
                    else None,
                ),
            })
    ...
    def __refresh_maoyan(self):
        ...
        if 'web-movie' in self._type:
            web_movie_url = f'{maoyan_url}/dashboard/web-heat'
    ...
    def __latest_tmdb_season(self, mediainfo: MediaInfo) -> int | None:
        ...
        tmdb_media_id = (
            mediainfo.media_id
            if normalize_media_source(mediainfo.media_source) == MediaSource.TMDB
            else None
        )
        if tmdb_media_id:
            try:
                latest_season = self.__latest_season_from_info(
                    TmdbChain().tmdb_seasons(tmdbid=tmdb_media_id)
                )
                if latest_season:
    ...
    @staticmethod
    def __get_http_json(url: str, headers: dict, source_name: str) -> dict:
        """请求普通 JSON 接口，并保留可诊断的失败信息。"""
        response = RequestUtils().get_res(url, headers=headers)
        if response is None:
            raise RuntimeError(f"{source_name} 请求失败（接口未返回成功响应）")

        status_code = getattr(response, "status_code", None)
        content_type = (getattr(response, "headers", {}) or {}).get("content-type", "")
        text = getattr(response, "text", "") or ""
        if status_code and status_code != 200:
            raise RuntimeError(f"{source_name} HTTP {status_code}")
        if not text.strip():
            raise RuntimeError(f"{source_name} 返回空内容")
        if "json" not in content_type.lower():
            raise RuntimeError(f"{source_name} 返回非 JSON 内容（{content_type or '未知类型'}）")
        try:
            return response.json()
        except (TypeError, ValueError) as err:
            raise RuntimeError(f"{source_name} JSON 解析失败: {err}") from err

    @staticmethod
    def __get_browser_json(
        page, path: str, source_name: str, allow_empty_status: bool = False
    ) -> dict:
        """在猫眼网页的 H5Guard 环境中请求加密接口。"""
        separator = "&" if "?" in path else "?"
        request_path = (
            f"{path}{separator}yodaReady=h5&csecplatform=4&csecversion=4.3.0"
        )
        ...
        if not result or result.get("status") != 200:
            detail = (result or {}).get("error") or f"HTTP {(result or {}).get('status', 0)}"
            raise RuntimeError(f"{source_name} 请求失败（{detail}）")
        if "json" not in (result.get("contentType") or "").lower():
            raise RuntimeError(f"{source_name} 返回非 JSON 内容")
        try:
            payload = json.loads(result.get("text") or "")
        except (TypeError, ValueError) as err:
            raise RuntimeError(f"{source_name} JSON 解析失败: {err}") from err
        if not payload.get("status") and not allow_empty_status:
            raise RuntimeError(f"{source_name} 返回失败状态")
        return payload
    ...
    def __get_url_info(self, movie_url, tv_urls, web_movie_url, num=10):
        ...
        if movie_url:
            try:
                res = self.__get_http_json(movie_url, headers, "电影票房榜")
                data = res.get('movieList', {}).get('list', [])
                def info(movie):
                    infos = movie.get('movieInfo') or {}
                    return {
                        "title": infos.get('movieName'),
                        "releaseInfo": infos.get('releaseInfo'),
                    }
                movies_list += [item for item in (info(i) for i in data) if item.get("title")][:num]
            except Exception as e:
                logger.error(f"获取电影票房榜失败: {str(e)}")

        if web_movie_url or tv_urls:
            context = None
            page = None
            try:
                ...
                if web_movie_url:
                    ...
                    platform_names = {3: "腾讯视频", 2: "爱奇艺", 1: "优酷"}
                    for platform_type, platform_name in platform_names.items():
                        try:
                            path = (
                                "/i/api/encrypt/dashboard/webHeatNetData"
                                f"?showDate={network_date}&platformType={platform_type}"
                                "&dateType=0&rankType=0"
                            )
                            res = self.__get_browser_json(
                                page,
                                path,
                                f"网络电影榜（{platform_name}）",
                                allow_empty_status=True,
                            )
                            if not res.get("status"):
                                calendar_key = (
                                    "calendarNetYOUKU" if platform_type == 1 else "calendarNet"
                                )
                                last_date = (
                                    (res.get(calendar_key) or {}).get("selectMaxDate") or ""
                                ).replace("-", "")
                                if not last_date:
                                    continue
                                path = (
                                    "/i/api/encrypt/dashboard/webHeatNetData"
                                    f"?showDate={last_date}&platformType={platform_type}"
                                    "&dateType=0&rankType=0"
                                )
                                res = self.__get_browser_json(
                                    page, path, f"网络电影榜（{platform_name}）"
                                )
                            data = res.get('dataList', {}).get('list', [])
                            for movie in data[:num]:
                                title = movie.get('movieName') or movie.get('name')
                                if title:
                                    movies_list.append({
                                        "title": title,
                                        "releaseInfo": movie.get('releaseInfo'),
                                        "platformDesc": movie.get('platformDesc') or platform_name,
                                    })
                        except Exception as err:
                            logger.error(f"获取网络电影榜（{platform_name}）失败: {err}")

                for tv_url, tv_num in tv_urls:
                    try:
                        query = parse_qs(urlparse(tv_url).query, keep_blank_values=True)
                        series_type = query.get("seriesType", [""])[0]
                        platform_type = query.get("platformType", [""])[0] or "0"
                        path = (
                            "/i/api/encrypt/dashboard/webHeatData"
                            f"?showDate={current_date}&seriesType={series_type}"
                            f"&platformType={platform_type}"
                        )
                        res = self.__get_browser_json(
                            page,
                            path,
                            f"网播热度榜（类型 {series_type}，平台 {platform_type}）",
                        )
                        data = res.get('dataList', {}).get('list', [])

                        for item in data[:tv_num]:
                            infos = item.get('seriesInfo') or {}
                        ...
                        logger.error(f"获取网播热度榜失败: {err}")
            except Exception as e:
                logger.error(f"初始化猫眼浏览器请求失败: {str(e)}")
            finally:
                try:
                    if page:
                        page.close()
        ...
        if movies_list:
            movies_list = list({item['title']: item for item in movies_list}.values())
