...
onUnmounted(() => clearTimeout(mediaPollTimer))
</script>
  ...
  <div class="eme-shell" :class="{ 'eme-shell--app': appPage }" @click="missingPicker.open = ''; missingActionPicker = false; mediaPicker.open = false; mediaSchedulePicker.open = false; confirmPicker.open = false">
    ...
    <main class="eme-main">
      ...
      <template v-if="active === 'media'">
        ...
        <section class="eme-card"><div class="eme-card-heading"><div><h3>定时清理</h3><p>默认关闭；启用后将按保存的媒体库和规则自动清理低质版本。请先停用 MediaEnhance 对应任务。</p></div><button class="eme-button primary" :disabled="busy || media.running" @click="mediaCommand('save', { config: media.config })">保存配置</button></div><label class="eme-switch-label"><input v-model="media.config.enabled" class="eme-switch-input" type="checkbox" role="switch" /><span class="eme-switch-track" aria-hidden="true" /><span>启用定时清理</span></label>
          ...
          <div class="eme-fields"><label>执行周期（cron表达式）<input v-model.trim="media.config.cron" placeholder="0 3 * * *" /></label></div></section>
...
.eme-overlay{background:rgba(34,32,54,.22);backdrop-filter:blur(2px)}
:global(.v-theme--dark .eme-overlay){background:rgba(0,0,0,.42)}
...
.eme-media-keep i{color:#38b86c;font-size:18px;vertical-align:middle}.eme-media-best-badge{display:inline-block;margin-left:6px;padding:2px 6px;border-radius:6px;background:rgba(44,176,112,.12);color:#239a69;font-size:11px;font-weight:700}
