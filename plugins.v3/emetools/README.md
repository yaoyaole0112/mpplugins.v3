# 订阅清理转存（MoviePilot 独立版）

插件卡片和插件内部使用同一张自带的 `icon.jpeg` 图标（来源：用户上传的蓝色星形图标）；远端插件清单从本仓库的原图地址加载图标。日志按功能输出扫描开始、候选与隔离结果、115 目录预览/清理、回收站核验与清空、逐规则文件转存，以及 Telegram 频道启动、订阅同步、消息命中与转发结果。频道轮询每分钟记录一次汇总，订阅消息不记录正文；不记录 Cookie、Bot Token、API Hash、验证码或 session。具体记录可在 MoviePilot 插件日志中按「订阅清理转存」筛选。
