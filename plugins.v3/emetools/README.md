...
左侧“媒体清理”独立移植 MediaEnhance 的扫描范围、七项质量比较规则（可启停、调整优先级）、重复版本列表、选择删除和定时自动清理。扫描使用 MoviePilot 中的 Emby 媒体库路径和 STRM 根目录，**两者须在 MoviePilot 容器内指向相同挂载路径**，否则拒绝扫描；不同于 EME，该实现不读取 Emby Item 的媒体流参数，文件名和同目录 JSON 是画质参数来源，无法判断质量时不会列入待删除。115 文件 ID 或 Cookie 不可用、云端删除失败时保留本地文件；删除后请求刷新 Emby。新定时任务默认关闭；启用前应先停用 MediaEnhance 同类任务。没有使用 MediaEnhance 服务或其数据库。原页面已停用的“115 同名格式去重”未重新启用。
