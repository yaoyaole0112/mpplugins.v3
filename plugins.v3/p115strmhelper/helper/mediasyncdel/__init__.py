...
from app.schemas.types import MediaType, MediaImageType, MediaSource, NotificationType
...
class MediaSyncDelHelper:
    ...
    def __get_transfer_his(
        ...
        # 删除电影
            ...
            transfer_history: List[TransferHistory] = self._query_transfer_history(
                tmdb_id=tmdb_id, mtype=mtype.value, dest=media_path
            )
        # 删除电视剧
        elif mtype == MediaType.TV and not season_num and not episode_num:
            msg = f"剧集 {media_name} {tmdb_id}"
            transfer_history: List[TransferHistory] = self._query_transfer_history(
                tmdb_id=tmdb_id, mtype=mtype.value
            )
        # 季处理为集（多版本季删除）
        elif (
            mtype == MediaType.TV
        ...
        # 删除季
            ...
            transfer_history: List[TransferHistory] = self._query_transfer_history(
                tmdb_id=tmdb_id, mtype=mtype.value, season=f"S{season_num}"
            )
        # 删除集
        elif mtype == MediaType.TV and season_num and episode_num:
            if (
    ...
    def _query_transfer_history(
        ...
        """按 V3 media_source/media_id 查询整理记录，兼容旧 tmdbid 入参。"""
        kwargs = {
            "mtype": mtype,
            "dest": dest,
            "season": season,
        ...
        if tmdb_id not in (None, "", 0, "0"):
            kwargs["media_source"] = MediaSource.TMDB
            kwargs["media_id"] = str(tmdb_id)
        history = self.transferhis.get_by(**kwargs)
        if history or tmdb_id in (None, "", 0, "0"):
            return history
        # 非 TMDB 主身份的历史：退回按目标路径查询
        if dest:
            return self.transferhis.get_by(mtype=mtype, dest=dest, season=season, episode=episode)
        return history
        ...
        if transfer_history:
            ...
            for transferhis in transfer_history:
                ...
                if title not in media_name:
                    logger.warn(
                        f"【同步删除】当前转移记录 {transferhis.id} {title} {getattr(transferhis, 'media_id', None) or getattr(transferhis, 'tmdbid', None)} 与删除媒体 {media_name} 不符，防误删，暂不自动删除"
                    )
                    continue
