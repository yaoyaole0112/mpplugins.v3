...
class TransferHandler:
    ...
    def _group_tasks_by_media(
        ...
        for task in tasks:
            if task.mediainfo:
                source = getattr(task.mediainfo, "media_source", None)
                media_id = getattr(task.mediainfo, "media_id", None)
                source_value = getattr(source, "value", source)
                if source_value and media_id:
                    identity = f"{source_value}:{media_id}"
                else:
                    identity = task.mediainfo.tmdb_id or task.mediainfo.douban_id
                key = (
                    identity,
                    task.meta.begin_season if task.meta else None,
                )
